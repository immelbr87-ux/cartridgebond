// ─────────────────────────────────────────────────────────────────────────────
//  CartridgeBond — Target Content Script
//  Injects a "buy used for less" comparison card on Target product pages.
// ─────────────────────────────────────────────────────────────────────────────

const CB_API_TGT  = 'https://script.google.com/macros/s/AKfycbybpmlpe6PtFsotY0iQ9CCCiYgsqJ8tyyLFib0pkqd8uyVKazgoyLiybfQZYmvC-xMd/exec';
const CB_SITE_TGT = 'https://www.cartridgebond.com';
...
function matchTgtGame(text) {
  const t = text.toLowerCase();
  for (const g of TGT_GAMES) {
    if (g.keys.some(k => t.includes(k))) return g;
  }
  return null;
}

function buildTgtCard(game) {
  const diff = (game.retail - game.price).toFixed(2).replace(/\.00$/, '');
  const pct  = Math.round(((game.retail - game.price) / game.retail) * 100);
  const buyUrl = `${CB_SITE_TGT}/index.html?game=${encodeURIComponent(game.title)}&mode=buy#widget`;

  return `
    <div class="cb-gs-wrap" id="cb-tgt-card">
...
function injectTgtCard(game) {
  if (document.getElementById('cb-tgt-card')) return;
  const anchor = document.querySelector('div[data-test="product-details-title"]');
  if (!anchor) return;
  const wrapper = document.createElement('div');
  wrapper.innerHTML = buildTgtCard(game);
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initTgt);
} else {
  initTgt();
}

// Target is a single-page app for navigation between related products
const tgtObserver = new MutationObserver(() => {
  if (!document.getElementById('cb-tgt-card')) initTgt();
});
tgtObserver.observe(document.body, { childList: true, subtree: true });
