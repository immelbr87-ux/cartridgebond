...
function matchGsGame(text) {
  ...
  for (const g of GS_GAMES) { if (g.keys.some(k => t.includes(k))) return g; }
  return null;
}

function findGsAnchor() {
  return document.querySelector('.trade-in-value, .tradeInValue, [class*="trade-in-value"], [class*="tradeIn"]') ||
...
const gsObserver = new MutationObserver(() => {
  if (!document.getElementById('cb-pill')) initGs();
});
