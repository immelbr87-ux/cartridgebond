// ─────────────────────────────────────────────────────────────────────────────
//  CartridgeBond — Best Buy Content Script
//  Injects a "buy used for less" comparison card on Best Buy product pages.
// ─────────────────────────────────────────────────────────────────────────────

const CB_API_BB  = 'https://script.google.com/macros/s/AKfycbybpmlpe6PtFsotY0iQ9CCCiYgsqJ8tyyLFib0pkqd8uyVKazgoyLiybfQZYmvC-xMd/exec';
const CB_SITE_BB = 'https://www.cartridgebond.com';
...
function matchBbGame(text) {
  const t = text.toLowerCase();
  for (const g of BB_GAMES) {
    if (g.keys.some(k => t.includes(k))) return g;
  }
  return null;
}

function buildBbCard(game) {
  const diff = (game.retail - game.price).toFixed(2).replace(/\.00$/, '');
  const pct  = Math.round(((game.retail - game.price) / game.retail) * 100);
  const buyUrl = `${CB_SITE_BB}/index.html?game=${encodeURIComponent(game.title)}&mode=buy#widget`;

  return `
    <div class="cb-gs-wrap" id="cb-bb-card">
...
function injectBbCard(game) {
  if (document.getElementById('cb-bb-card')) return;
  const anchor = document.querySelector('.shop-product-title, .pdp-header') ||
                 document.querySelector('h1.sku-title, div.sku-title h1')?.closest('div');
  if (!anchor) return;
  const wrapper = document.createElement('div');
  wrapper.innerHTML = buildBbCard(game);
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initBb);
} else {
  initBb();
}

// Best Buy is a single-page app for navigation between related products
const bbObserver = new MutationObserver(() => {
  if (!document.getElementById('cb-bb-card')) initBb();
});
